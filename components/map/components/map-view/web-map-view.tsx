...
interface WebMapViewProps extends MapViewProps {
  ...
  moveToLocationRef?: React.MutableRefObject<
    ((location: { lat: number; lng: number }) => void) | null
  >;
  scale?: number;
}
...
export function WebMapView({
  ...
  const updateCurrentLocationMarkerRef = useRef<(() => void) | null>(null);
  const currentLocationRef = useRef<{ lat: number; lng: number } | null>(null);
  const isLoadingLocationRef = useRef<boolean | undefined>(false);
  ...
  const updateMarkers = useCallback(() => {
    ...
    markers.forEach(({ id, lat, lng, type }) => {
      try {
        let marker;
        const imageSize = new kakaoMaps.Size(scaledWidth, scaledHeight); // 스케일에 따른 마커 이미지 크기
        const imageOption = { offset: new kakaoMaps.Point(scaledOffsetX, scaledOffsetY) }; // 마커 이미지의 옵션 (하단 중앙 기준)

        // 이스터에그일 경우 파란색 커스텀 SVG 마커 사용
        if (type === 'EASTER_EGG') {
          const markerImage = new kakaoMaps.MarkerImage(easterEggMarkerSvg, imageSize, imageOption);
  ...
  const initMap = useCallback(() => {
    ...
    const handleCenterChanged = () => {
      ...
      // 지도 중심점이 변경될 때 현재 위치 마커 다시 표시
      // 약간의 딜레이를 주어 지도 이동이 완료된 후 마커 표시
      setTimeout(() => {
        if (mapRef.current && updateCurrentLocationMarkerRef.current) {
          updateCurrentLocationMarkerRef.current();
        }
      }, 100);
    };

    kakaoMaps.event.addListener(mapRef.current, 'center_changed', handleCenterChanged);
  ...
  useEffect(() => {
    if (mapRef.current && level !== undefined) {
  ...
  const updateCurrentLocationMarker = useCallback(() => {
    ...
    const isLoading = isLoadingLocationRef.current ?? false;

    if (
      !mapRef.current ||
      !window.kakao?.maps ||
      !mapsLoadedRef.current ||
      isLoading ||
      !currentLoc
    ) {
    ...
    try {
      ...
      const position = new kakaoMaps.LatLng(currentLoc.lat, currentLoc.lng);
  ...
  // updateCurrentLocationMarker를 ref에 저장
  useEffect(() => {
    updateCurrentLocationMarkerRef.current = updateCurrentLocationMarker;
  }, [updateCurrentLocationMarker]);

  // currentLocation과 isLoadingLocation을 ref에 저장
  useEffect(() => {
    currentLocationRef.current = currentLocation || null;
    isLoadingLocationRef.current = isLoadingLocation;
  ...
  // currentLocation이나 isLoadingLocation이 변경될 때 현재 위치 마커 업데이트
  useEffect(() => {
    if (mapRef.current && window.kakao?.maps && mapsLoadedRef.current) {
      updateCurrentLocationMarker();
    }
  }, [currentLocation, isLoadingLocation, updateCurrentLocationMarker]);

  // 현재 위치로 이동하는 함수
  const moveToLocation = useCallback((location: { lat: number; lng: number }) => {
    if (!mapRef.current || !window.kakao?.maps || !mapsLoadedRef.current) return;
